...
function install_dmp() {
    local download_urls=(
        "https://gh.llkk.cc/https://raw.githubusercontent.com/xiaochency/DMP3.0/refs/heads/master/dmp"
        "https://github.dpik.top/https://raw.githubusercontent.com/xiaochency/DMP3.0/refs/heads/master/dmp"
        "https://ghfast.top/https://raw.githubusercontent.com/xiaochency/DMP3.0/refs/heads/master/dmp"
    )
...
function start_dmp() {
    ...
    if [ -e "$ExeFile" ]; then
        nohup "$ExeFile" -bind ${PORT} -dbpath ${CONFIG_DIR} -level ${LEVEL} >dmp.log 2>&1 &
    else
        install_dmp
        #create_dstmp_config
        if [ -e "$ExeFile" ]; then
